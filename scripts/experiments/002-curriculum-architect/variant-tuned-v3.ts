/**
 * Variant D — tuned curriculum architect v3.
 *
 * Builds on Variant C (journey-voice, de-bucketed time, end-goal preservation,
 * attention-unit modules, source granularity). Adds one principle to fix the
 * Transformer-shrinking regression observed in C:
 *
 *   Total curriculum scope follows the learner's stated depth and time
 *   availability — not the physical size of the immediate source. A small
 *   source with an ambitious learner calls for expansive surrounding context.
 *   A large source with a casual learner calls for a curated slice.
 *
 * Output JSON shape unchanged.
 */
...
export function createTunedCurriculumArchitectV3(
  topic: string,
  goal: string,
