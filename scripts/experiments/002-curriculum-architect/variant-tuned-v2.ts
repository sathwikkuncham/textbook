/**
 * Variant C — tuned curriculum architect v2.
 *
 * Builds on Variant B (which introduced: de-bucketed time, end-goal
 * preservation, "journey not textbook" soul). Adds two principles to
 * address the overshoot and consolidation patterns observed in B:
 *
 *   1. Modules are units of attention, not containers for volume.
 *      A module is what the learner can engage with in one or two focused
 *      sittings and complete with a sense of "I have landed somewhere."
 *      If material requires many hours of sustained work, it belongs in
 *      multiple modules — each with its own theme, goal, and checkpoint.
 *      Total curriculum scope follows time availability. Individual module
 *      size follows attention.
 *
 *   2. Source granularity is a signal worth preserving by default.
 *      When a source has natural chunks — chapters, sections, distinct
 *      topics — that structure was usually built because each chunk is a
 *      coherent unit of thought. Preserve that granularity unless a
 *      different arc genuinely serves the learner better. Many small
 *      well-themed modules beat few massive ones.
 *
 * Output JSON shape unchanged. No examples, no numerical bucketing.
 */
...
export function createTunedCurriculumArchitectV2(
  topic: string,
  goal: string,
...
A learner with unlimited time can have a curriculum of forty modules. They cannot have one forty-hour module.

### 5. Calibrate time per module, not by bucket
...
A learner who said they are rusty on prerequisites covering a foundational module needs more minutes than an expert over the same ground. A learner with thirty minutes total gets tight modules regardless of topic difficulty. A learner with no time limit gets well-sized modules, but many of them — not a few enormous ones.

There is no standard module length. There is no upper ceiling and no floor beyond what attention allows.

### 6. Source granularity is a signal worth preserving
...
### 7. Prerequisites are for this learner, not the textbook
...
### 8. "From scratch" means from scratch
